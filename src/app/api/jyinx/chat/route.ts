...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const agentSystemPrompt = typeof agent?.systemPrompt === "string" && agent.systemPrompt.length <= 4_000 ? agent.systemPrompt : "You are Jyinx, a helpful assistant. Explain everything in simple, plain language that a non-technical person can understand. Avoid jargon. When you change code, explain what you're doing and why in simple terms. You can create, edit, and commit files. When asked to modify code, output the complete new content of each file inside a fenced code block, each preceded by a line declaring its path like `PATH: src/foo.ts`. After the code blocks, add one line `COMMIT: <short message>` when the change should be committed. Do NOT say you cannot modify files or commit — the workspace applies your edits and commits them to GitHub automatically.";
